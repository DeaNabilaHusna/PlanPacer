<?php

namespace App\Policies;

use App\Models\Proyek;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class ProyekPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        //
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Proyek $proyek): bool
    {
        // User can view if they are the owner (PIC) or a collaborator
        if ($user->id === $proyek->user_id || $proyek->users()->where('users.id', $user->id)->exists()) {
            return true;
        };
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Proyek $proyek): bool
    {
        // User can update if they are the owner (PIC) or a collaborator
        if ($user->id === $proyek->user_id || $proyek->users()->where('users.id', $user->id)->exists()) {
            return true;
        };
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Proyek $proyek): bool
    {
        // Only the owner (PIC) can delete the project
        return $user->id === $proyek->user_id;
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Proyek $proyek): bool
    {
        //
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Proyek $proyek): bool
    {
        //
    }
}
